...
#' @examples
#' \dontrun{
#' rgenesconvergedPlot(tree, primates, "Mouse", pos=17, t=3, type="abs")
#' }
#'
...
#' @import ggplot2
#' @export
rgenesconvergedPlot <- function(tree, phydat, spe, pos, t, type=c("abs", "score")) {

  convNodes <- getConvergent(tree, phydat, spe, pos, t, type=type)
  ...
  tplot <- ggtree::ggtree(tree, branch.length = tree$edge.length, aes(color=group)) +
